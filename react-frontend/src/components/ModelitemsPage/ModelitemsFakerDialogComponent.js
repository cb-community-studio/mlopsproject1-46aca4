...
import modelitemsFakerFactory from "./modelitemsFakerFactory";
...
const ModelitemsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = modelitemsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("modelitems").create(elem));
...
export default connect(null, mapDispatch)(ModelitemsFakerDialogComponent);
